///
/// \file task2.cpp
/// \brief Реализация функционала задач по геометрии
/// 
...
/// 2. Запрашивает задачи по геометрии с сервера
///
Task2::Task2(QTcpSocket* socket, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Task2),
    m_socket(socket)
{
    ...
    // Подключаем сокет
    connect(m_socket, &QTcpSocket::readyRead, this, &Task2::onSocketReadyRead);

    // Запрашиваем задачи с сервера
    m_socket->write("get_problems geometry\r\n");
}

///
/// \brief Деструктор класса Task2
...
///
void Task2::on_checkButton1_clicked()
{
...
///
void Task2::on_checkButton2_clicked()
{
...
///
void Task2::on_checkButton3_clicked()
{
...
///
void Task2::on_checkButton4_clicked()
{
...
///
void Task2::checkAnswer(int problemIndex, const QString& answer)
{
...
///
void Task2::on_backButton_clicked()
{
    this->close();
    emit backRequested();
}
